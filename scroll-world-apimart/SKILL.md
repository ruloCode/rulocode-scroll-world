---
name: scroll-world-apimart
description: >
  Build an immersive scroll-scrubbed "fly through the world" landing page for any
  industry or brand using the APIMart generation API (api.apimart.ai). As the visitor
  scrolls, a pre-rendered camera flies from outside each scene into its interior, then
  flows on to the next scene with NO cuts — one continuous connected flight
  (Emons-style isometric diorama world, or any art direction you pick). The skill
  interviews the user for the topic, the story beats/sections, and brand kit, then
  generates cohesive scenes + seamless camera clips through APIMart's async REST API
  (image models + Seedance/Kling video models with first/last-frame locking) and wires
  a portable, framework-agnostic scroll-scrub engine. Use when the user wants a
  "3D world" / "browse-through-the-industry" hero, a scroll cinematic, a diorama
  landing, or to turn a business into a scrollable world.
allowed-tools: Bash, Read, Write, Edit, AskUserQuestion, Skill
---

# scroll-world (APIMart edition)
...
cuts. The visuals are AI-generated through the **APIMart REST API**; the page just scrubs
pre-rendered video by scroll position. This is the same technique behind Apple's
scroll-through product pages — the camera genuinely moves, scroll only drives time.
...
The value of this skill is the generation pipeline, the prompts, and the seam method —
not the framework.
...
1. **APIMart API key.** All generation goes through `https://api.apimart.ai` with
   `Authorization: Bearer $APIMART_API_KEY`. If the env var is unset, ask the user to
   create a key at https://apimart.ai/keys and export it — you cannot create one for
   them. Verify with a cheap call before batching (e.g. submit one z-image-turbo task,
   or just check that an intentionally-bad request returns 401 vs 400).
2. **curl + jq** on `$PATH` (the whole pipeline is plain REST; no vendor CLI to install).
...
5. **(Optional) Codex CLI** — if `codex` is on `$PATH` (≥ 0.125) and
   ...
   through Codex's built-in `image_gen` (gpt-image-2) billed to the user's ChatGPT
   subscription instead of APIMart balance — offer it at Step 1.6, command in Step 2.
   Absence just removes the option.
6. Caveats:
   - macOS ships **bash 3.2** (no `declare -A`); don't use associative arrays.
   - **Everything is async**: POST returns a `task_id`; poll
     `GET /v1/tasks/{task_id}` every ~5 s until `status` is `completed` or `failed`.
     Generations take minutes — always run batches detached (background) and poll,
     never a foreground blocking call.
   - **Every result URL expires** (generated images ~24 h, videos carry an
     `expires_at`, uploads live 72 h). `curl` results down to disk the moment a task
     completes; never store an APIMart URL as the artifact.
   - **Local files are passed by upload**: `POST /v1/uploads/images` (multipart
     `file=@...`, ≤ 20 MB, jpeg/png/webp/gif) returns a URL valid 72 h that any
     `image_urls` / `image_with_roles` field accepts. Base64 data-URIs are also
     accepted inline but the docs recommend the upload endpoint — use it.
   - Video models differ in accepted params (Kling has `mode`, Seedance has
     `resolution`/`size`) and in whether they support first/last-frame conditioning
     at all — before batching, confirm the chosen model's schema against
     https://docs.apimart.ai and see the Step 4 model table.
   - **Seedance 2.0 generates audio by default** (`generate_audio: true`) — always
     pass `false`; the page is muted and audio wastes money and bytes.
...
2. **Brand kit** — offer two paths, pick one:
   - The user hands you palette + name + tone directly (if they give a website URL,
     eyeball it and extract the palette yourself).
...
5. **Mobile version — ALWAYS ask this; never silently generate both.** Ask as a
   ...
   *"Want a mobile-optimized version too? The mobile version is a second camera chain
   ...
   landscape film — which roughly doubles the video spend (state the estimated cost)."*
   Options: "Desktop only" / "Desktop + mobile (native 9:16 — ~2× video cost)". The
   cost must be stated to the user, not just implied.
   ...
   - **Yes** → render the parallel 9:16 portrait chain and ship it as the mobile variants
     ...
     the full mobile QA (Step 8). Budget ~2N-1 extra video gens + moderation re-rolls.
     **Never ship the centre-crop as the mobile version by default** — if budget can't
...
6. **Budget — tiers shown by cost, decided before anything renders.** Present the
   ...
   - **Video tier** (roster only — every option frame-locks seams, Step 4):
     ...
     |---|---|---|
     | Draft / previz | `doubao-seedance-2.0-mini` (720p) | cheapest |
     | Fast middle | `doubao-seedance-2.0-fast` (1080p) | between |
     | Standard (default) | `doubao-seedance-2.0` (1080p) | baseline |
     | Alternate | `kling-v3-omni` | different look + content filter |
     ...
     unprompted when the budget reads tight. Draft and Standard are the **same model
     family**, so the previz translates almost 1:1 to the final render.
   - **Stills source** (only offer if the Codex CLI is present, Step 0.5):
     APIMart image models (spends balance) vs **Codex `image_gen`** — gpt-image-2
     billed to the ChatGPT subscription (zero APIMart spend; counts toward Codex
     usage limits; 1536×1024 output — exactly 3:2). Stills are plain PNGs handed to
     the video model as the first frame, so the video chain is indifferent to their
     source. Command in Step 2. **One source for all N stills of a build** — models
     render with slightly different character, and mixing sources across scenes reads
     as style drift, same reason the video chain uses one model.
   - **Calibrate costs, don't guess.** Every completed task's status payload reports
     the real charge (`cost` in USD and/or `credits_cost`) — this makes calibration
     exact. Run ONE still and ONE video first, read the `cost` fields off
     `GET /v1/tasks/{id}`, extrapolate to the full run
     (`N × still_cost + (2N−1) × video_cost × [2 if mobile] × 1.15`), state it, and
     warn the user if it exceeds what they said they're comfortable spending. An
     HTTP **402** mid-run means the balance ran out — recoverable (finished clips
     survive; resume after top-up) but ugly — the whole point of this step is that
     the user decides *before* the spend.
...
model **`flux-2-pro`** (crisp, native `3:2` size support, and takes `image_urls`
references — useful to lock style to an approved scene). Cheap draft tier:
**`z-image-turbo`** (also native 3:2, 1K/2K). Use **`gemini-3-pro-image-preview`**
(aka `nano-banana-pro`) only if the brief is character/cartoon-heavy — probe its `size`
support with one call before batching (docs list `1:1`/`16:9` examples; verify `3:2`
lands as requested, otherwise generate `16:9` and pad/crop to 3:2 locally).
...
- Endpoint: `POST /v1/images/generations` with
  `{"model":"flux-2-pro","prompt":…,"size":"3:2","resolution":"2MP"}`
  (z-image-turbo takes `"resolution":"2K"`) → returns `task_id` → poll
  `GET /v1/tasks/{task_id}` → download the result URL immediately (it expires in ~24 h).
  Batch script in `references/pipeline.md` §1.
- Run all N concurrently, detached.
- **Codex stills variant** (if chosen at Step 1.6 — subscription-billed, zero
  APIMart spend): same prompt files, same byte-identical preamble, generated by Codex's
  built-in `image_gen`:
  ...
  ~1–3 min per image; run a few in parallel, not all N at once. Everything downstream
  (cohesion review, knockout, dives) is unchanged.
- A generation may fail transiently (5xx / rate limit 429) — re-roll that one
...
- **Review the stills before continuing.** They must read as one cohesive world (same
  ...
  approved scene in `image_urls` to lock style.
...
frame-lock a seam: every chained clip must accept a **first-frame** image, and
connectors also need a **last-frame** image. On APIMart that is the
`image_with_roles` parameter — `[{"url":…,"role":"first_frame"},
{"url":…,"role":"last_frame"}]` — and that capability, not preference, is the selection
rule. **Skip anything whose image inputs are reference-only** (`reference_image` role or
plain style conditioning): it can only *condition* a generation, not *continue* a shot,
so it physically can't hold a seam. All video models submit to
`POST /v1/videos/generations` and poll on `GET /v1/tasks/{task_id}`:

| Model | first/last frame | Notes |
|---|---|---|
| `doubao-seedance-2.0` (default) | ✓ / ✓ (`image_with_roles`) | Full chain (legs + connectors). `resolution: "1080p"`, `size: "16:9"`, duration 4–15 s. **`generate_audio: false` always** (defaults true — costs more and you'll mute anyway). Supports `return_last_frame: true` → the task result hands you the actual last-frame URL, which feeds the next leg/connector without an ffmpeg-extract + upload round-trip. Its content filter is the touchy one (see Gotchas). |
| `kling-v3-omni` | ✓ / ✓ (`image_with_roles` recommended) | Full chain. Different provider = different content filter — the sanctioned moderation fallback. Check its current mode/duration options in the docs before batching. |
| `doubao-seedance-2.0-fast` | ✓ / ✓ | Faster/cheaper sibling, same frame-locking. Middle tier. |
| `doubao-seedance-2.0-mini` | ✓ / ✓ | Cheap draft tier that keeps frame-locking (720p). The previz tier: run the whole chain here first, then re-render final legs on the full model — still seamless, so it translates directly. |

(`kling-v2-6` also frame-locks — two `image_urls` = first+last — but **only in
`mode: "pro"`**; std mode takes one image and can't hold a connector's end. It's not in
the default roster; if the user asks for it, force `pro` and accept 5/10 s durations.)
...
- **One model for all chained clips.** Each renderer has its own motion/color/grain
  ...
  the moderation fallback for a single stubborn clip (Gotchas) — a slight character
  shift on one 5s connector beats a missing connector.
- Default to `doubao-seedance-2.0`; honor a user's stated preference **only if the model
...
- The pipeline scripts take the model as `$VMODEL` with per-model params already cased
...
then each leg's **first_frame = the previous leg's ACTUAL last frame** (use Seedance's
`return_last_frame` URL, or extract with ffmpeg + upload), prompt *"continue gliding
smoothly FORWARD into [scene i], never pulling back"* (or an expressive mid-leg move
under the motion-handoff contract — see **Camera grammar** below), and **no last_frame
role** — a last-frame of a wide establishing shot forces the camera to pull back, which
is the #1 cause of stutter. Result: every seam is frame-identical **and** the camera
never reverses. There are **no connectors** (skip Step 5) — the legs ARE the journey.
Wire each leg as a section clip with `connectors: []` and a small `crossfade` (~0.08).
Even without a last-frame the legs still arrive at distinct rooms (the prompt steers the
content). Cost: strictly **sequential** (can't parallelize) and slower; interiors trip
the content filter, so build in re-rolls (3 attempts/leg).
...
**`doubao-seedance-2.0`**), first_frame = the scene still (uploaded).
...
- Params (seedance 2.0): `resolution: "1080p"`, `size: "16:9"`, `duration: 8`,
  `generate_audio: false`, `return_last_frame: true`.
- Run concurrently, detached, then download each result the moment its task completes
  (URLs expire). Re-roll individual failures. Keep the raw 1080p sources — you need
  their frames next.
...
For each connector between dive_i and dive_{i+1}:
  first_frame = the LAST frame of dive_i's rendered video
  last_frame  = the FIRST frame of dive_{i+1}'s rendered video
```
...
- **Last frames**: if the dives ran with `return_last_frame: true` (Seedance), the task
  result already carries the last-frame URL — use it directly as the connector's
  `first_frame` (it IS the rendered pixels). Otherwise extract + upload:
...
# upload each PNG:  POST /v1/uploads/images  (multipart file=@…) → URL valid 72 h
```

- **First frames** always come from extraction (there's no return_first_frame): extract
  from the **rendered video**, upload, use the upload URL as the connector's `last_frame`.
- Either way, **also extract the frames locally** — you need them to eyeball the
  handoffs (Step 4 grammar) and for QA diffing (Step 8).

Generate the connector (`duration: 5` is plenty). Connectors need a `last_frame` role,
so the model must accept it — any roster model does:

```bash
# body (seedance 2.0):
{ "model": "doubao-seedance-2.0", "prompt": <connector prompt>,
  "image_with_roles": [
    {"url": <dive_i last frame URL>,    "role": "first_frame"},
    {"url": <dive_i+1 first frame URL>, "role": "last_frame"} ],
  "resolution": "1080p", "size": "16:9", "duration": 5, "generate_audio": false }
# POST /v1/videos/generations → task_id → poll GET /v1/tasks/{task_id} → download
```
...
Insurance: the model lands *close* to the last-frame target but not always pixel-perfect,
so the engine still applies a **short crossfade** (a few frames) at each seam.
Frame-matched endpoints + a small crossfade = no visible cut. Never skip the actual-frame
handoff and rely on the crossfade alone; a big content jump can't be hidden by a crossfade.
...
connector (`size: "9:16"`), encoded **720 wide (`scale=720:-2`), `-g 4`** (more
keyframes = cheaper seeks — phone decoders' seek cost scales with GOP length), crf 23 —
wired as `clipMobile` / `connectorsMobile`, with each portrait dive's first frame
extracted as the section's `stillMobile` poster (Step 7). The engine serves them
automatically on phones and falls back to the desktop clip when absent. The 16:9
centre-crop `encm()` encodes (pipeline.md §6) are a **fallback only** — for when budget
can't cover the portrait chain — and shipping them must be called out to the user, never
silent. If the user chose desktop-only, skip this — the engine still hardens phone
scrubbing regardless (seek-coalescing, iOS priming), so the page degrades gracefully
...
- **Mobile — full checklist only if the user opted into the mobile version (Step 1.5).**
  ...
  - Only if the crop **fallback** shipped (no budget for the portrait chain): portrait
...
## Gotchas (hard-won + API-specific)

- **Seam pop** → connector endpoints were the diorama stills, not the neighbouring
  clips' actual frames. Always use real rendered frames (Step 5) — `return_last_frame`
  URLs or ffmpeg-extracted + uploaded PNGs.
- **Seam stutter / camera "jumps backward"** → even with frame-matched seams, if the
  ...
  pull-back, no last_frame role); see Step 4.
...
- **Expired asset URL (403/404 on download, or a generation rejecting an input URL)** →
  APIMart URLs are temporary: generated images ~24 h, videos per `expires_at`, uploads
  72 h. Download results the moment tasks complete; if a chain step stalls past the
  window, re-upload the local frame and use the fresh URL.
- **HTTP 402** → balance ran out. Finished clips survive; top up and resume the batch
  (only re-roll the tasks that never completed).
- **HTTP 429** → rate limit; back off and retry that one task. Launching ~5–6 gens at
  once is fine; much more invites 429s — stagger or re-roll.
- **Task stuck in `processing`** → keep polling every 5 s up to a generous timeout
  (20 min for video); only then re-submit. Never re-submit early — you'll pay twice.
- **Moderation false-positives (task `failed` with a content/moderation reason)** → the
  video content filter flags perfectly innocuous clips, especially **bedroom, pool,
  spa/wellness** contexts and trigger words like "bed", "pool", "waterfall", "wine",
  "swim". It's partly the prompt wording and partly the reference frames. Fixes, in
  order: (1) re-roll — it's often non-deterministic and passes on the 2nd–3rd try;
  (2) strip trigger words and add "empty, unoccupied, no people, no figures,
  architectural, tasteful"; (3) regenerate just that clip on **`kling-v3-omni`** with
  the same first/last frames — a different provider's filter often passes what Seedance
  blocks. Expect a slight render-character shift on that one clip (each model has its
  own grain/motion feel); for a 5s connector behind a crossfade that usually beats
  option (4): set the connector slot to `null` — the engine crossfades that seam
  directly (optional connectors), so the page still completes. Budget extra spend/time
  for these re-rolls on interiors/real-estate content.
- **Unexpected audio track / higher cost on Seedance 2.0** → you forgot
  `generate_audio: false` (it defaults **true** on this model). `-an` on encode strips
  it, but you still paid for it — always pass false.
- **Kling v2-6 rejects two images** → two-image frame-lock needs `mode: "pro"`; std
  takes one image only. Roster models (`kling-v3-omni`, seedance 2.0) don't have this trap.
...
- **Seam pop only where you "saved money"** → you swapped models mid-chain, or used a
  first-frame-only setup where a connector needs a `last_frame`. One model for the whole
  chain; the only cheap tier is `doubao-seedance-2.0-mini`, which keeps frame-locking so
  it stays seamless. (Any model with reference-only image inputs can't hold a seam at
  all — Step 4.)
- **White-box scenes** → the still models return a solid bg when prompted so; either
  match the page bg to it or knock it out (Step 3).
...
- `references/pipeline.md` — copy-paste batch scripts for the whole run (upload →
  generate → poll → download → extract frames → connectors → encode → mobile encode),
  bash-3.2-safe, curl+jq only.
...
- APIMart API docs: https://docs.apimart.ai (uploads: `/v1/uploads/images`; images:
  `/v1/images/generations`; videos: `/v1/videos/generations`; polling: `/v1/tasks/{id}`).
