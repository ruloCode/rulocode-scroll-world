...
- `MOBILE` — yes/no. **Always asked** (SKILL Step 1.5), presented to the user
  with the ~2× video-cost stated. Yes = the **native 9:16 portrait chain** (pipeline §6b):
  portrait renders of every dive/connector + `clipMobile`/`connectorsMobile`/`stillMobile`
  wiring + the full mobile QA. The §6 crop encodes are a no-budget stopgap only.
- `VIDEO_TIER` — draft (`doubao-seedance-2.0-mini`) | fast (`doubao-seedance-2.0-fast`) |
  standard (`doubao-seedance-2.0`, default) | alternate (`kling-v3-omni`). Chosen by cost
  at SKILL Step 1.6, with the calibrated total estimate (from real per-task `cost`
  fields) stated before anything renders.
- `STILLS_SOURCE` — apimart (`flux-2-pro` default / `z-image-turbo` draft /
  `gemini-3-pro-image-preview` for character-heavy briefs; spends balance) | codex
...
- Params: `size: "3:2"`, `resolution: "2MP"` (flux-2-pro) / `"2K"` (z-image-turbo).

## Leg prompt — architecture A, continuous forward take (Step 4)

first_frame = previous leg's ACTUAL last frame (leg 0: the first scene's still) — via
Seedance's `return_last_frame` URL or an extracted + uploaded PNG. **No last_frame
role.** The bolded clauses are the motion-handoff contract — keep them verbatim; the
mid-leg move is where the expression goes.
...
first_frame = the scene still (solid-bg version), uploaded.
...
Params by chain model (SKILL Step 4 table): seedance 2.0 —
`resolution: "1080p", size: "16:9", duration: 8, generate_audio: false,
return_last_frame: true`; kling-v3-omni — check its current param set in the docs,
`duration: 10` for dives. Same for architecture-A legs.

## Connector clip prompt (Step 5)

first_frame = dive_i LAST frame, last_frame = dive_{i+1} FIRST frame — both from the
RENDERED videos (extracted + uploaded, or the dive's `return_last_frame` URL), never
the stills.
...
seedance 2.0: `resolution: "1080p", size: "16:9", duration: 5, generate_audio: false`.
Connectors need the `last_frame` role → use a roster model that accepts it (Step 4).
